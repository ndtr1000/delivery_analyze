with company_1 as (
    select user_id, order_id, time::date, action
    from user_actions
...
select 'Компания № 1' as ads_campaign,
    round((sum(price) - 250000.0)/250000.0*100,2) as ROI
from (
    select user_id, t1.order_id, unnest(product_ids) as product_id
    from (
        select user_id, order_id, time::date, action
        from company_1 
        where order_id not in (select order_id from user_actions where action='cancel_order')
    ) as t1 left join orders using(order_id)
) as t2 inner join products using(product_id)

union 

select 'Компания № 2' as ads_campaign,
    round((sum(price) - 250000.0)/250000.0*100,2) as ROI
from (
    select user_id, t3.order_id, unnest(product_ids) as product_id
    from (
        select user_id, order_id, time::date, action
        from company_2
        where order_id not in (select order_id from user_actions where action='cancel_order')
    ) as t3 left join orders using(order_id)
) as t4 inner join products using(product_id)
