...
select 'Кампания № 1' as ads_campaign,
    round(avg(avg_price),2) as avg_check
from (
    select user_id, avg(price) as avg_price
    from (
        select user_id, order_id, sum(price) as price
        from (
            select user_id, order_id, time, unnest(product_ids) as product_id
            from (
                select user_id, order_id, time
                from company_1
                where order_id not in (select order_id from user_actions where action='cancel_order') 
                    and time between '2022-09-01' and '2022-09-07'
            ) as t1 inner join orders using(order_id)
    ...
    from (
        ...
        from (
            ...
            from (
                ...
                where order_id not in (select order_id from user_actions where action='cancel_order') 
                    and time between '2022-09-01' and '2022-09-07'
            ) as t5 inner join orders using(order_id)
        ) as t6 inner join products using(product_id)
        group by user_id, order_id
    ) as t7
    group by user_id
) as t8
