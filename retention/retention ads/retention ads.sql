with company_1 as (
    select distinct user_id, time::date as action,
...
select 'Кампания № 1' as ads_campaign, start_date, day_number, retention
from (
    select first_action as start_date, 
        action - first_action as day_number,
        round(rt::decimal/max(rt) over(),2) as retention
    from (
...
select 'Кампания № 2' as ads_campaign, start_date, day_number, retention
from (
    select first_action as start_date, 
        action - first_action as day_number,
        round(rt::decimal/max(rt) over(),2) as retention
